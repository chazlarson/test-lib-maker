...
createbasevideo () {
    ...
    if [ -f $FILE ]; then
        ...
        echo "Creating $FILE..."
        ffmpeg -loop 1 -i testpattern.png -c:v libx264 -t 900 -pix_fmt yuv420p -vf scale=$2 $FILE
        echo "$FILE created"
        echo "==================="
    fi
}
...
createtestvideo () {
    ...
    ffmpeg -i "$3.mkv" -i sounds/15-min-audio.m4a -c copy -map 0:v:0 -map 1:a:0 "test_movie_lib/$1 $2/$1 [WEBDL-$4 H264 AAC 2.0]-BINGBANG.mkv"
}
